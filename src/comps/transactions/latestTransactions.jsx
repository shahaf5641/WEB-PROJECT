...
const LatestTransactions = () => {
  ...
  const fetchTransactions = async (page) => {
    try {
      const response = await fetch(`https://explorer.mtw-testnet.com/transactions/?page=${page}&limit=5`);
      if (!response.ok) {
  ...
  return (
    <div className="container mx-auto p-2 rounded-xl backdrop-blur-md">
      <h2 className={`text-2xl font-semibold mb-4 ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>Latest Transactions</h2>
      <TransactionTable transactions={transactions} tokenAddressToName={tokenAddressToName} />
      <div className="flex justify-end py-4">
        <Link to="/transactions" className="relative group p-2">
          <button
            type="button"
          ...
          >
            ...
            <span className="translate-x-2">View all transactions</span>
          </button>
        </Link>
      </div>
    </div>
